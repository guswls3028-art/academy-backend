...
- [ ] Worker 동시성 모니터링 (Max limit 준수 확인)

## 📊 최종 현실 합일점 (운영 안정성 + 가성비 + 성능)

### 구조 (완료)
- ✅ Redis-only progress
- ✅ 완료 TTL 없음
- ✅ Tenant namespace
- ✅ Progress endpoint 분리

### Excel (완료)
- ✅ Batch 조회 (Tuple IN 방식)
- ✅ bulk_create
- ✅ Chunked transaction (200개 단위)
- ✅ Composite index 추가 필요 (`idx_student_tenant_name_phone`)

### 인프라 (권장)
- ✅ 최소 RDS small (현실적 가성비)
- ✅ AI worker concurrency 2 (안정성)
- ✅ DB_CONN_MAX_AGE = 15 (연결 점유 최소화)
...
### 핵심 원칙
1. **진행 상태는 절대 DB로 가지 않는다** ✅
2. **Excel은 절대 row-by-row 처리하지 않는다** ✅
3. **Worker는 무한 확장하지 않는다** ✅
4. **DB는 최소 small 이상** ✅

### 단계별 확장 전략
- **500명**: RDS small + 현재 구조
- **3K명**: RDS medium + 인덱스 최적화
- **10K명**: RDS large + Read/Write 분리 준비
- **10K+**: Aurora 고려
