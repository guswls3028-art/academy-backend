...
**키가 채팅/문서에 노출되었다면 즉시 AWS 콘솔에서 해당 키 비활성화·삭제 후 새 키 발급(키 로테이션) 하세요.**

---

## 0.5 배포 시 버그 방지 (워커 잘못 뜸 / SSH 실패)

- **풀배포·캐시·노캐시**는 **한 세션에서 같은 AWS 키(루트 또는 ECR+EC2+ASG 권한 있는 계정)로 끝까지** 실행하세요.
- 중간에 admin97 등 **다른 키로 바꾼 뒤** `full_redeploy`를 돌리면, **다른 계정**으로 인스턴스/ECR을 보게 되어:
  - API/워커 인스턴스가 안 보임 → SSH 실패
  - ASG/ECR이 다른 계정 것 → 워커가 잘못 뜨거나 이미지 불일치
- **빌드 서버 수동 중지**만 admin97 키 사용. 풀배포는 루트(동일 계정) 키로.
...
- 현재 AWS 계정, SSH 키 존재 여부, 실행 중 인스턴스·ASG를 확인합니다. 문제 없으면 같은 env로 `full_redeploy.ps1` 실행.
