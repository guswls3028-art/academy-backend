...
### SSOT 신설
- `docs/domain/hexagonal-cutover-policy.md` (필독)
...
## 22b. V1.2.x 영상 후속 (2026-05-10 / 2026-05-11)

봉인 직후 영상 파이프라인 대정비 + 코드 정리. 상세는 별도 메모 `project_video_pipeline_overhaul_2026_05_10.md`.

- **2026-05-10 영상 파이프라인 대정비** — SSL stale-conn fix + R2 병렬 업로드 + c6g.4xlarge VCPU=8 + daemon/long path 폐기 + recover_dead_video_jobs cron 추가. 80분 영상 60분 → 17분.
- **2026-05-11 영상 구조 정합 audit + IaC 보강 (현 문서 갱신 시점)**:
  - `terminate_batch_job` footgun rename: `batch_control.terminate_aws_batch_job` (raw aws id) vs `batch_submit.terminate_video_job` (video_job UUID).
  - 사장 코드 제거: `academy/application/video/handler.py` (ProcessVideoJobHandler / SQS fast-ack 잔재) → `cancellation.py` 로 CancelledError 만 분리. `reconcile_video_processing` 커맨드 / `try_claim_video` / `try_reclaim_video` / `VideoProgressAdapter` (apps 버전) / `transcoder.prepare_output_dirs` / `job_count_backlog` / `worker_asg/user_data/video_worker_user_data.sh` 일괄 삭제.
  - long path 잔재 청소: `.env.example` / `scripts/v1/ensure-video-long.ps1` / `*_long.json` 3 종 / `core/ssot.ps1` `evidence.ps1` `resources/batch.ps1` `jobdef.ps1` `eventbridge.ps1` `deploy.ps1` `cleanup-legacy.ps1` `run-resource-inventory.ps1` `update-api-env-sqs.ps1` `core/sync_env.ps1` `verify-video-batch-connection.{ps1,sh}` `deploy-api-and-verify-workers.ps1` 의 LONG 분기 제거. SSM 잔재 자동 청소.
  - PATH 헤더 stale 일괄 정리 (`apps/domains/video/**/*.py` 32 + `academy/adapters/video/{duration,validate,config}.py`).
  - `infrastructure-optimization.md` / `v1.1.0.md` / `v1.1.1.md` 헤더에 ⚠️ HISTORICAL 명시.
  - cron 7종 정합: `video-cron-jobs.md` "6종" → "7종" 정정. `state-transitions.md` B3 daemon → AWS Batch 갱신. `worker_asg/README.md` Video ASG 폐기 명시. `deployment-architecture.md` jobdef timeout 2h → 6h 정정.
  - IaC 보강: `eventbridge.ps1` 에 detect-stuck / recover-dead / purge-raw / cleanup-orphan 4 rule ensure 추가 + `templates/batch/` 에 ops_job_definition_*.json 4 종 추가 + `settings.base` 에 7 rule + 7 jobdef env override 노출. SSOT_JobDef / SSOT_EventBridgeRule 인벤토리 확장.
  - `batch_main._handle_term` 시그널 핸들러에 `close_old_connections()` 추가 — v1.2.0 사고 패턴 (RDS Proxy stale conn) 의 시그널 경로 누락 보완.
