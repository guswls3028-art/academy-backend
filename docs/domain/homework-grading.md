...
문제 수에 맞는 만점과 차시 공통 합격 정책을 설정한 뒤 학생 점수를 입력하는
현재 계약이다.

1. **강의 → 차시 → 과제**에서 과제를 만들고 각 과제의 만점을 지정한다.
2. 같은 차시의 **과제 정책**에서 모든 과제에 적용할 합격 기준을 지정한다.
...
| 과제 정책 | 차시 공통 | `HomeworkPolicy`, `cutline_mode`와 `cutline_value` |
...
정책은 한 차시의 모든 현재 과제에 공통 적용된다.

- `PERCENT`: 각 과제의 `score / max_score`를 기준으로 판정하므로 만점이
  서로 다른 과제에 권장한다. 퍼센트 반올림 단위는 기존 정책을 따른다.
- `COUNT`: 원점수 커트라인이다. 공통 커트라인은 현재 과제 중 가장 낮은
  만점을 초과할 수 없다.
- 새 과제의 만점은 현재 원점수 커트라인보다 낮을 수 없다.
- 기존 과제의 만점은 현재 원점수 커트라인이나 이미 입력된 최고 점수보다
...
| GET/PATCH | `/homeworks/{id}/` | 과제별 만점 조회·변경과 기존 1차 점수 동기화 |
| GET/PATCH | `/homework/policies/` | 차시 공통 합격 정책 조회·변경 |
| PATCH | `/homework/scores/quick/` | 성적표 셀 점수 저장. 서버 만점을 정본으로 사용 |
| GET | `/results/session-scores/` | 성적표 과제 메타와 각 셀에 과제별 만점 반환 |

기존 과제에 `default_max_score`가 없으면 100으로 읽는다. 이미 과제 메타에는
...
- 공통 정책: `apps/domains/homework/serializers/core.py`,
