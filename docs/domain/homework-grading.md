...
차시 성적표의 편집 초안은 조교별로 유지한다. 시험 점수 변경은 같은 시험 공유
차시 전체를 독점하지만, 과제만 편집하는 초안은 `(enrollment_id, homework_id)`
셀 집합이 서로 겹치지 않으면 여러 조교가 동시에 저장할 수 있다. 같은 학생의
같은 과제 셀, 시험 변경이 섞인 초안, 같은 사용자의 다른 브라우저 탭은
`409 SCORE_EDIT_LOCKED`로 차단한다. 빈 초안은 다른 직원의 입력을 막지 않는다.
과제 부분 저장은 현재 요청 셀에 대한 활성 초안 소유권을 다시 확인한 뒤
반영하므로 서로 다른 셀의 안전한 동시 작업과 같은 셀의 덮어쓰기 방지가 함께
유지된다.
