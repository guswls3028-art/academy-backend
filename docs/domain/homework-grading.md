...
무중단 전환은 두 릴리스로 수행한다. 첫 호환 릴리스는 `results.0020`의
`client_id`와 브라우저별 유일조건을 적용하되 앱은 빈 client 슬롯 하나만 사용한다.
구버전 API도 컬럼 기본값 `""`을 사용하므로 같은 사용자에게 두 번째 행을 만들지
않는다. 이 호환 릴리스가 모든 API 인스턴스에 반영된 뒤에만 앱이 브라우저별 행을
사용하도록 전환한다. 따라서 구버전·신버전 API가 겹치는 동안에는 기존 단일
편집기 계약을 유지하고, 최종 릴리스 뒤 셀 단위 동시 편집을 연다.
