...
- 텍스트가 있는 쪽과 이미지 전용 쪽이 섞여 있으면 텍스트 쪽의 문항이 모두
  검출되어도 이미지 전용 쪽이 문항 모드에서 누락될 수 있으므로 전체 쪽을
  페이지 단위로 보존하고 이미지 세그멘테이션은 재시도하지 않는다. 전체가
  이미지 전용인 PDF의 세그멘테이션 경로는 그대로 유지한다.
