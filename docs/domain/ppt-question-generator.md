...
텍스트 계층이 없는 스캔 PDF는 이미지 세그멘테이션을 시도한다. 문항을 찾지
못했거나 일부 페이지에만 문항 후보가 있으면 페이지 단위로 변환하고 응답의
`mode=page`로 프런트엔드가 이를 알린다. 교사는 같은 PDF의 `직접 자르기`에서
페이지별 영역을 직접 지정·수정·정렬해 PPTX를 다시 만들 수 있다. 브라우저가
선택 영역을 이미지로 만든 뒤 기존 이미지 생성 API로 제출하므로 수동 좌표나
원본 PDF 페이지를 별도 정본 문항으로 저장하지 않는다.
...
- 분석 대상이 3쪽 이상인데 첫 문항 분석에서 후보가 잡힌 페이지가 절반
  미만이면, 일부 문항만 담긴 결과를 성공으로 내보내지 않고 전체 페이지를
  보존한다. 이 경우 이미지 세그멘테이션 재시도는 하지 않는다.
...
직접 자르기 실패 후에는 선택한 원본과 영역이 현재 화면에 남아 재시도할 수
있고, 새 PDF를 선택하면 이전 원본의 좌표를 지운다. 페이지 단위 자동 결과는
완료 알림에 표시되며 교사가 직접 자르기로 복구할 수 있다.
