...
7. 학생 이름의 가나다순은 데이터베이스 기본 locale에 맡기지 않는다. PostgreSQL은
   이름 키에 Unicode 코드포인트 순서인 `COLLATE "C"`를 명시하고, 그 뒤에 학생·
   enrollment·행 ID 보조키를 둔다. 따라서 운영 DB가 `en_US.UTF-8`이어도 `가`,
   `가가`, `각`, `간`, `나` 순서와 동명이인 위치가 API마다 같아야 한다.
...
| 학생·수강·차시 수강·영상 현황 목록 | 기본 학생명 가나다순, 학생/enrollment/행 ID 보조키 | 검색·삭제 탭·강의·차시·영상·tenant 필터 후 전체 정렬 뒤 페이지 |
...
학생 목록의 `ordering`이 없거나 허용되지 않은 값이면 `name,id`로 복구한다.
이름 내림차순은 `-name,-id`를 사용하며, 다른 학생 열을 정렬할 때도 이름과 ID를
보조키로 붙인다. 수강·차시 수강·영상 시청 현황과 권한 대상처럼 별도 정렬 UI
없이 학생 행을 반환하는 API도 학생명과 ID 보조키를 기본으로 사용한다. 상세
열기·선택·재조회는 이 표시 순서를 바꾸지 않는다.

PostgreSQL 회귀는 학생 목록, 강의 수강생, 전체 차시 수강생, 특정 차시 활성
수강생, 영상 권한 수강생의 SQL에 같은 `COLLATE "C"`가 적용되고, 위 고정 한글
표본이 페이지 분할 전에 같은 순서로 정렬되는지 검증한다.
