...
청구 스냅샷을 만들기 직전에 후보 `StudentFee`와 연결 `FeeTemplate`을 순서대로
잠그고 활성 여부, 시작·종료월, 청구 주기, template 금액, 개별 조정액과 할인을
최신 행으로 다시 평가한다. 잠금보다 먼저 완료된 비용 수정은 새 청구에 반영되고,
잠금 뒤의 수정은 청구 트랜잭션이 끝날 때까지 대기한다.
...
대신 건너뜀으로 처리한다. 일회성 비목의 월 교차 경합과 비용 수정 대 청구 생성은
PostgreSQL 행 잠금 테스트로 검증한다.
