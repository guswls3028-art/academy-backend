...
- `03_자체양식_문제검수본.hwpx` is generated from the `python-hwpx==4.2.0` editor-compatible package skeleton, and its `Preview/PrvText.txt` is synchronized with the section text. The API, AI CPU worker, and deterministic tools worker all pin the same writer dependency.
...
| `production-confirmed` | HWPX package compatibility | The 2026-07-27 damaged-file incident was traced to the legacy hand-built ZIP/XML package. Deployment run `30269167091` replaced it, and a tenant-1 non-persisting production response passed package reopen, schema validation, ZIP ordering, required header/settings/content references, and preview/section text synchronization checks. |
