...
**선생님메모 자동 보강 (2026-04-09~):**
- Solapi 템플릿 본문 = `#{선생님메모}\n#{사이트링크}` 구조
- 실제 카카오톡 표시 정보는 **선생님메모 값**에 모두 포함되어야 함
- `template.body`에 변수 블록(`#{학생이름}`, `#{장소}` 등)이 **하나도 없으면**, 핵심 컨텍스트(학생이름, 장소, 날짜, 시간 등)를 **자동 앞에 추가**
- 선생님이 body에 직접 변수 블록을 사용하면 자동 추가 안 함 (커스텀 우선)
- 영향 범위: `build_unified_replacements()` (자동발송) + `build_manual_replacements()` (수동발송)
