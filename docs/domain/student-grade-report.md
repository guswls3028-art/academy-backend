...
학생 홈의 **제출 할 일**은 성적과 별개다. 과제 행의 `submission_state`는
`needs_submission`(제출 필요), `awaiting_review`(성공한 파일/온라인 제출이 있고
그 증거를 교사가 아직 채점하지 않음), `reviewed`(통과·보완 완료)를 구분한다.
이전 불합격 점수는 보존하되 새 제출이 검토 대기 중이면 제출 할 일에서 제외한다.
파일 업로드 실패·제거로 활성 제출 증거가 없으면 다시 제출 필요가 된다. 교사의
명시적 점수 저장은 당시 활성 제출 증거의 지문을 기록하므로 불합격 검토 뒤에는
재제출 할 일이 돌아오고 정책 재계산만으로 검토 완료가 되지 않는다. 기존 점수의
명시적 `NOT_SUBMITTED` 저장도 같은 검토 지문을 적용하므로, 이미 제출한 증거를
교사가 미제출로 확정했다면 새 제출 전까지 할 일이 유지된다.
검토 지문이 없는 과제는 현재 활성 제출 증거를 검토 대기로 표시한다. 과거 제출과
채점의 정확한 선후관계는 복원할 수 없어, 이 이력은 교사의 명시적 재검토로 확정한다.
시험 행의 `submission_pending=true`는 새 온라인 답안이 접수되어 채점 중임을 뜻하며
이전 불합격 원점수를 보존한 채 재시험 제출 할 일만 잠시 제외한다. 처리 실패 시
