...
- 학생·학부모: `GET /api/v1/student/grades/`를 통해 시험 카드의 오답 확인 상태와
  성장 그래프 구성을 읽는다.
...
`score_trend`, `score_comparison`, `lecture_average` 표시로 저장한다. 이는 편집 가능한
초깃값이므로 관리자는 같은 API에서 나머지 섹션을 다시 켜거나 순서를 변경할 수 있다.
