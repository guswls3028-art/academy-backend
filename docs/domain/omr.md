...
- 단답형 전용: `0/20`까지 한 페이지에 생성할 수 있으며 객관식 ROI는 0개, 자동 객관식 채점은 불가
...
단답형은 최대 20문항의 별도 작성 영역이며 자동 버블 인식 대상이 아니다. 시험 문항 번호는 전체 문항 기준으로 유지하되, OMR 엔진은 `choice_count`까지만 객관식 ROI를 만든다. 객관식 전용 시험의 표시용 `단답형 공간`은 문항 번호·배점·채점 대상이 아니다.
