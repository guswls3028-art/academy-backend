...
- `origin_id` exact 필터는 미리보기 확인 발송 또는 직접 발송 한 번의 요청만
  조회한다. 직접 발송 API는 프런트가 보낸 UUID `request_id`(없으면 서버 생성)를
  `batch_id`로 반환하고, 학생·학부모 대상별 outbox와 worker 로그에 같은 값을
  보존한다. 수신 번호가 한 건도 없거나 큐/예약 접수가 0건이면 2xx 성공으로
  표시하지 않는다.
