...
포함된다. 시험 만점 수정도 정규 시험에 대해 같은 재채점을 실행한다. 합격 점수
수정은 진행도·클리닉 판정을 같은 트랜잭션에서 다시 계산하며, 실패하면 설정
...
정답·배점·답안·시험 설정이 달라지면 확정을 거부하고 새 미리보기를 요구한다.
기존 OMR 제출이나 출처가 다른
...
학생·선택 자녀의 시험 목록/상세는 제출 상태를 `submission_pending`으로 함께
반환한다. 같은 tenant와 활성 수강 범위의 접수·dispatch·추출·답안 준비·채점 중
제출이 있으면 true이며, 실패·식별 필요·대체됨·완료만 있거나 제출이 없으면 false다.
`has_result`와 응시 횟수, 기존 답안·점수·재응시 제한은 그대로 유지한다. 학생 홈은
이 필드로 접수된 시험을 다시 해야 할 예정 시험과 구분하며 실패를 완료로 숨기지 않는다.
추가 필드이므로 구 클라이언트와 호환되고 데이터 마이그레이션은 없다.
`test_parent_exam_child_selection.py`가 실제 제출201 뒤 학생/선택 자녀 재조회,
다른 자녀 격리와 처리·실패·완료 상태 전이를 검증한다. UI 계약은 frontend
`docs/STUDENT-PARENT-APP-CONTRACT.md`를 따른다.
...
다르면 stale 변경으로 거부한다.
개별 문항 배점과 기대값은 둘째 자리로 반올림하지 않고 원수로 비교한다.
예를 들어 `100/6` 배점을 사용자가 `16.6667`로 직접 고치면 작은 차이도
명시적 변경으로 저장하며, 그 사이 다른 화면의 미세 변경도 덮어쓰지 않는다.
이는 문항 입력의 정밀도 보존이며 결과 합계의 소수 둘째 자리 반올림과
위 합계 허용 오차는 유지한다. 기존 클라이언트의 원배점 기대값 요청과
호환되고, 잘못된 범위·stale 요청은 기존 점수와 배점을 보존한다.
