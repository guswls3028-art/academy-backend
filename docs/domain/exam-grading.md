...
업로드 화면은 한 진입점에서 다음 세 자료 형태를 받는다.

- **문제+해설 한 파일**: 형식과 관계없이 안전한 자료 원본 한 개
- **문제 파일만**: 답 표시가 없는 안전한 자료 원본 한 개
- **문제·해설 두 파일**: 형식과 관계없이 문제 원본과 선생님 해설 원본을 각각
  보관. PDF/이미지 문제 + HWP/HWPX 해설 조합은 번호 자동 연결도 시도한다.
...
문제지와 해설지가 분리된 Ymath 자료는 자동 번호 연결을 원할 때 **답 표시가 없는
학생용 문제지 PDF/이미지**를 주 파일로, 같은 시험의 **선생님 해설 HWP/HWPX**를
선택 파일로 함께 올린다. 그 밖의 안전한 형식 조합도 거부하지 않고 두 원본을
각각 보존하며, 지원되는 문제 원본의 문항 분리는 계속 진행하고 해설은 직접
연결·검수한다.
