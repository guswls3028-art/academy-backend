...
- **학생별 등수**: 서버 석차와 같은 1차 점수, 학교·강의, 득점률·평균 대비,
  합격/보충 대상/보충 완료·현재 대표 결과의 오답 문항
...
분포는 석차 계약과 같은 1차 점수를 사용해 재시험 뒤에도 수업 당시 분석이 변하지
않고, 학생별 답안과 오답 문항은 현재 대표 결과를 사용한다. 문항 통계도
현재 대표이면서 채점 완료된 시도만 집계해 재시험 횟수가 응시자 수처럼 중복되지
...
분리한다. 공동 등수는 기존 `compute_exam_rankings` 계약을 그대로 따른다. 응시 5명 미만은 표본 확인을
