...
- **Tools** keeps ASG min/desired=1/1 so Excel/PPT/PDF work has no instance cold-start, and AWS/SQS CloudWatch alarms add burst capacity for deterministic conversion queues. Scale-in uses the same visible+in-flight+delayed backlog guard and cannot go below ASG min. Its Docker container writes instance-scoped streams to `/academy/tools-worker` with the same 30-day retention and fail-closed IMDSv2 identity rule as AI.
