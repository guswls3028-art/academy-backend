# DB Connection Budget (V1.3.0)

## 현재 설정
| 항목 | 값 |
|---|---|
| RDS 인스턴스 | db.t4g.large |
| max_connections | 400 |
...
- Django persistent DB connection is disabled in production (`DB_CONN_MAX_AGE=0`)
  after the 2026-06-12 idle-connection incident; do not raise it without a
  fresh RDS connection budget review.
