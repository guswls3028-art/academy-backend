...
- **zone에 수동 CNAME을 넣지 않는다.** (→ academy-frontend.pages.dev 등으로 넣으면 Error 1014 발생)
- Pages 커스텀 도메인 추가는 Phase 2에서 스크립트로만 수행.
...
- 기존 zone에 수동 CNAME이 있으면 제거한 뒤, **Workers & Pages → academy-frontend → Custom domains**에 `새도메인.co.kr`, `www.새도메인.co.kr` 추가.
...
## 참고: 오늘(sswe.co.kr) 적용 요약

- **Phase 1**: `add-cloudflare-zone.ps1 -Domain "sswe.co.kr"` → 네임서버 2개 전달 → 가비아 등록 완료 알림.
- **Phase 2**:  
  - `pages-add-custom-domain-sswe.ps1`(이후에는 `pages-add-custom-domain.ps1 -Domain "sswe.co.kr"`로 대체 가능)  
...
| Pages 커스텀 도메인 추가(범용) | `backend/scripts/pages-add-custom-domain.ps1` |
