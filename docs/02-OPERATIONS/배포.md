...
| 방식 | 설명 | 권장 |
|------|------|------|
| **정석 배포** | `scripts/v1/deploy.ps1` — 로컬에서 이미지 빌드·ECR 푸시, SSM `/academy/api/env` 갱신, API Launch Template UserData(`/opt/api.env`) 반영, instance refresh. API는 `/opt/api.env` + ECR 이미지로 기동. | ✅ 권장 |
| **레거시(원격 자동 배포)** | §10 — `api-auto-deploy-remote.ps1` / `deploy_api_on_server.sh`. API EC2에서 2분마다 `origin/main` 감지 시 레포 기준 빌드·재기동. 인스턴스에 Git 레포 필요. | 정석과 병행 가능, 정석 우선 |
...
## 10. API 서버 Git 기준 자동 배포 (원격 제어) — 레거시

API EC2에서 **2분마다** `origin/main` 변경을 감지하면 `scripts/deploy_api_on_server.sh`로 배포하고 **구이미지(dangling) 제거**까지 수행.  
원격에서 ON/OFF는 SSM Send-Command로 가능. **인스턴스에 레포가 없으면** `-RepoUrl` 로 한 번 클론 후 ON 하면 됨.

| 목적 | 명령 (로컬 PowerShell) |
|------|-------------------------|
| **자동 배포 ON**  | `pwsh scripts/v1/api-auto-deploy-remote.ps1 -Action On -AwsProfile default` |
| **자동 배포 OFF** | `pwsh scripts/v1/api-auto-deploy-remote.ps1 -Action Off -AwsProfile default` |
| **crontab 상태**   | `pwsh scripts/v1/api-auto-deploy-remote.ps1 -Action Status -AwsProfile default` |
| **수동 배포 1회**  | `pwsh scripts/v1/api-auto-deploy-remote.ps1 -Action Deploy -AwsProfile default` |

레포가 인스턴스에 없으면 On/Deploy 시 로컬 `git remote origin` URL로 자동 클론 후 진행.
...
# 자동 배포 ON (2분마다 main 변경 시 배포 + 구이미지 제거)
