...
SSM을 올린 뒤 **API 서버에 같은 .env를 적용**하려면:
...
- academy-api EC2에 SSH → `aws ssm get-parameter ... > /home/ec2-user/.env`  
- **반드시 컨테이너 재생성** (아래 8.2.1). `docker restart` 는 **예전 env 유지**하므로 새 .env가 안 먹힘.
...
**API 서버(ec2-user)에서 한 번에 실행:**

```bash
IMG=$(docker inspect academy-api --format '{{.Config.Image}}' 2>/dev/null); \
docker stop academy-api 2>/dev/null; docker rm academy-api 2>/dev/null; \
docker run -d --name academy-api --restart unless-stopped --env-file /home/ec2-user/.env -p 8000:8000 "$IMG"
```

또는 repo가 있으면: `bash /home/ec2-user/academy/scripts/refresh_api_container_env.sh`
