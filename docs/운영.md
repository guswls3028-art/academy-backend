...
```bash
# SSM에서 받아서 .env 덮어쓰기 (값 있는 SSM 전제)
aws ssm get-parameter --name /academy/workers/env --with-decryption --query Parameter.Value --output text --region ap-northeast-2 | sed 's/\t/\n/g' > /home/ec2-user/.env
# 빠진 키 있으면 아래처럼 추가 (실제 값으로 교체)
# echo 'R2_ACCESS_KEY=값' >> /home/ec2-user/.env
# echo 'R2_SECRET_KEY=값' >> /home/ec2-user/.env
# echo 'R2_ENDPOINT=값' >> /home/ec2-user/.env
# echo 'R2_PUBLIC_BASE_URL=값' >> /home/ec2-user/.env
# echo 'AWS_ACCESS_KEY_ID=값' >> /home/ec2-user/.env
# echo 'AWS_SECRET_ACCESS_KEY=값' >> /home/ec2-user/.env
bash /home/ec2-user/academy/scripts/refresh_api_container_env.sh
```

SSM에 R2 등이 없으면 위에서 주석 풀고 값 넣은 뒤 `refresh_api_container_env.sh` 실행.
