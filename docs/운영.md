...
#### 8.2.2 복구: R2·VIDEO_BUCKET 등이 이미 빠진 경우

SSM으로 덮어쓴 뒤 CLOUDFLARE_R2_*, VIDEO_BUCKET_NAME 등이 없어졌다면:

1. **로컬(Windows) .env에 R2·VIDEO_BUCKET 등이 다 들어 있는지** 확인한 뒤, `.\scripts\sync_api_env_lambda_internal.ps1` 실행 → 로컬 .env를 SSM에 올리고, API 서버는 **병합**하므로 기존에 서버에만 있던 값은 유지된다.  
2. **이미 서버 .env가 망가진 상태**면: 로컬 .env를 완전히 채운 다음 `.\scripts\sync_api_env_lambda_internal.ps1` 한 번 돌리면, SSM에 풀 내용이 올라가고, 병합 시 SSM에 있는 게 적용된다. (이때 서버 .env에는 R2가 없으므로 병합해도 SSM 내용이 곧 전체가 됨.)  
3. 또는 API 서버에서 **수동 복구**: R2/VIDEO_BUCKET 값을 알고 있으면 `echo 'CLOUDFLARE_R2_ENDPOINT=...' >> /home/ec2-user/.env` 등으로 추가한 뒤 `bash scripts/refresh_api_container_env.sh` 실행.
