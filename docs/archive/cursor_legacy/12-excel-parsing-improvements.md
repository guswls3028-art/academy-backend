...
| 규칙 1차 + AI 워커 2차 컬럼 분류 | ✅ 반영 | `_rule_guess_parent_score`, conf 0.6~0.9 시 `excel_schema_infer` AI 호출, AI conf < 0.8이면 업로드 실패 |
| 학원별 매핑 학습 | 🔲 Optional | `_get_academy_parent_mapping` 확장 포인트, 실제 저장은 TODO |

→ 상세 설계: `docs_cursor/13-excel-parsing-final-design.md`
