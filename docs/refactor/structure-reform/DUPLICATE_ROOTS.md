...
| Student create | `[PARTIAL ACCOUNT GRAPH COMPLETED]` `StudentViewSet.create`, `bulk_create`, `approve_registration_request`, `get_or_create_student_for_lecture_enroll`, Excel worker, E2E direct API setup | Admin create modal, teacher create sheet, signup approval, lecture enroll Excel, student Excel import, E2E fixtures | `students.services.create_student_account` for Parent/User/Student/Membership graph; callers keep validation, duplicate policy, responses, and dispatch | View-level user/parent/membership creation logic removed from active create roots; registration approval orchestration now has a service root; full import orchestration service still pending | P1 | `[DONE]` account graph, parent notice, membership, welcome flag regression, frontend contract build, registration approval orchestration; `[TODO]` import orchestration service snapshot |
