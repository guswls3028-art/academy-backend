...
| Frontend source import files | 1051 | files scanned for import boundary snapshot |
| Frontend source text files | 1395 | app/domain moves need automated boundaries |
...
| Frontend cross-app imports | 6 | remaining role-app imports of admin internals |
| Frontend role-app admin imports | 6 | teacher/student app imports of `@admin/*` internals |
...
- Captured verification status: these counts include the session-enrollment,
  ...
  contract/status slice, and the tools timer download contract slice. Re-run the snapshot
