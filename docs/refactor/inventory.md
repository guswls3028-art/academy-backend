...
| Backend cross-domain internal imports | 594 | semantic snapshot script, direct imports into models/services/views/api/serializers |
...
- NotificationLog `source_tenant_id`, OMR detected-answer `exam_question_id`,
  ...
  selection debt to the strict-safe subset; the remaining 33 warnings are 28
  `[ALLOWED]` integer-FK candidates plus 5 `UNORDERED_FIRST` instances in
...
- Captured verification status: these counts include the session-enrollment,
  ...
  `cross_domain_internal_import=594`, and `domain_infra_import=82`.
