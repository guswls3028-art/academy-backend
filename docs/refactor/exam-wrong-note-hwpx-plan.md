...
구현되었다. HWPX는 원본 문제·해설 이미지를 보존하면서 제목·정답·풀이 메모·
추가 메모를 편집 가능한 문단으로 제공한다. 박스 직접 수정·분할·병합과 원본
수식/필기 픽셀 자체의 편집 가능한 개체 변환은 아직 구현되지 않아 남은 범위를
`refactor/`에 둔다. 현재 계약은
...
- 두 단 PDF에서 섹션 제목의 `1.`과 실제 1번이 겹쳐 dispatcher가 2번부터
  반환한 경우에는, 같은 열에 정렬된 PDF 네이티브 `1.`·`2.` 앵커가 각각
  하나이고 같은 페이지의 확정 문항이 둘 이상일 때만 1번 박스를 복구한다.
  열이 다르거나 앵커가 중복되면 추측하지 않고 교사 검수 실패로 남긴다.
...
- 현재 오답노트 출력은 tools worker가 비동기로 만드는 PDF/HWPX다. HWPX의
  제목·정답·풀이 메모·추가 메모는 편집 가능한 문단이고 문제·해설 원본은
  충실도를 우선해 이미지로 포함한다.
