...
**상태:** 주요 민원 수리와 운영 배포 완료. 추가 온라인 제출·서술형 민원의 현재 배포 및 남은 증거는 §9를 따른다. 장애 알림 수신처 설정은 별도 미해결 항목이다.
**갱신 기준:** 2026-09-24 KST. 현재 실행·운영 SHA/run은 §9, 앞선 안정화 배치는 §8, 첫 배치 출발점·결과는 §2·§7.
...
## 9. 추가 제출·서술형 저장 민원 — 2026-09-24 현재 상태

**현재 배포:** backend PR [491](https://github.com/guswls3028-art/academy-backend/pull/491)은
main `6f7017395730fa87aa71d66c1c7bff7206e64f3d`에 병합됐고 공식
[배포 35574136656](https://github.com/guswls3028-art/academy-backend/actions/runs/35574136656)가
성공했다. 성공 manifest는 main `e2cc3da9463737d0c684fc923bc17a13756a60fb`,
API digest `sha256:fe265581a37f38eaecc66d27263017de967b66b8668d1ffa0decd7bc275656ca`다.
frontend PR [566](https://github.com/guswls3028-art/academy-frontend/pull/566)의
제품 수정은 main `9f763d6682e53ae6eabb8bb8b619bf662be91e52`에 병합됐다.
첫 공식 실행 35576560746은 시험 제출 후 예정 시험 검사의 과도한 제목 일치
assertion 때문에 20/21에서 중단됐고 운영 승격되지 않았다. 실제 예정 시험 행과
같은 제목의 합법적인 재시험·교정 행을 구분하도록 PR
[567](https://github.com/guswls3028-art/academy-frontend/pull/567)의 검사만 고쳐
main `7be78d6b024a0f70d6364b16f90835798c5ccd77`에 병합했다.
공식 [실행 35606905460](https://github.com/guswls3028-art/academy-frontend/actions/runs/35606905460)은
같은 산출물의 격리 21개·정리0, 운영 배포와 읽기 전용 검사가 성공했다.
후속 학생 홈의 시험 목록 로딩 경합 수리 `735053b94ae104212c7fa4a87c73a6fffc922f34`도
공식 [실행 35888605850](https://github.com/guswls3028-art/academy-frontend/actions/runs/35888605850)에서
격리 21개·skip/flaky/실패0, 양쪽 QA tenant/user 정리0, 운영 배포·읽기 전용 검사가
성공했다. 이 실행의 `development-release.json`은 위 backend manifest/digest와
frontend SHA를 함께 기록한다. PC/모바일 영상은 각각 690초 재생·갱신2회,
player error0이었다. 특정 격리 시나리오의 성공이며 실제 모든 사용자 환경의
무중단을 보장하지 않는다. 2026-09-24 KST에 두 공개 도메인의 cache-buster
`/version.json`을 별도로 읽어 모두 `735053b94ae104212c7fa4a87c73a6fffc922f34`를
확인했다. 이는 버전 경로의 현재 readback이며 전체 자산의 재검사는 아니다.

**남은 구분:** 원민원의 제출 대상이 시험인지 과제인지 확인되지 않았다. 시험의
정상 접수·처리 중 상태가 예정 시험 할 일에 남던 경로는 위 배포에서 수리했다.
과제의 무점수 제출은 접수 후 할 일에서 빠지지만, 기존 미통과 점수가 있는
재제출은 교사 판정 전까지 미통과로 남는 현재 계약이다. 실제 과제 사례의
학생·과제·제출 시각 및 검수 이력 없이는 같은 장애로 확정하거나 점수를
자동 변경하지 않는다.

**별도 메모 검증 완료:** 2026-09-24에 frontend `735053b9`의 공식 동일 산출물과
위 backend release/digest로 격리 개발 환경에서 실제 관리자·조교·학생·학부모
역할을 검증했다. 관리자 PC1366/1100/390과 조교390의 강의 메모 저장·reload,
같은 강의 다음 차시 반영·다른 강의 분리, 학생 공통 메모 보존, 두 편집자의
409 충돌·초안 복구, 학생/학부모 403을 확인했다. 일회용 tenant/user와
R2/process/listener·토큰·활동 감사 잔여0, SSM·로컬 포트 종료를 확인했다.
현재 frontend head의 모의 브라우저9개는 조회/저장 실패·재시도·초안 보존까지
통과했다. 운영 `hakwonplus.com`의 읽기 전용 PC/390 세 화면씩은 첫 표시
대기 실패 뒤 같은 버전 재검증에서 6개 모두 통과했고 업무 쓰기 전달0이었다.
인증 요청1건과 외부 요청 차단은 업무 쓰기0과 구분한다. 영수증은
`C:\academy\_artifacts\student-lecture-memo-0920\memo-live-qa-receipt.json`,
`memo-browser-evidence.json`, `production-readonly-result.json`에 보존한다.
이 검사는 위 21개 릴리스 시나리오와 별개이며 실제 모든 사용자 환경의 성공을
보장하지 않는다.

아래 §9.1까지는 위 배포에 이르기 전 후보·판단 기록이다. 당시의 pending/HOLD는
현재 배포 상태를 대체하지 않는다.
...
당시 Memo/DS 작업 `01a0be43-7495-7e51-aee3-5368244c74c8`과 배포 소유권을
조율했다. 아래 순서는 후보 단계의 계획이며 위 실제 배포 영수증을 우선한다.
...
해당 최종 후보의 추가 상태 경계 테스트는 아래 증거로 완료했으며 초기3PASS와 구분한다.
...
수리했고 실제 조교 권한 API12개가 통과했다. 원민원과 동일 원인인지는 미확정이다.

### 9.1 후속 정밀도 경계와 통합 후보

BE PR491의 `7fccd43e7`은 공식 CI35568845470 전체 성공, FE PR566의
Memo main `6fb6e0c2` 통합 후보 `f39b4c5d6`은 Quality35570455192 성공이다.
아래 후속 제품 수정 전의 증거이므로 새 정확한 head의 CI를 다시 확인한다.
공유 배포는 여전히 Memo/DS 소유이며 인계 전 main/공유 QA를 변경하지 않는다.

원래 배점을 정확히 표시하는 것에 더해 사용자가100/6을16.6667로 직접 고치는
경계도 재현했다. 화면은0.001 이하 차이를 버려 새 배점을 보내지 않았고,
서버는 원배점 기대값을 둘째 자리로 반올림해 정상 변경을 충돌로 거부했다.
개별 배점·기대값을 그대로 보존하고 실제 수치 차이를 변경으로 처리한다.
합계의 둘째 자리 반올림·0.01 허용 오차·기존 점수·권한/tenant 경계는 유지한다.
잠금 뒤 기대값도 정확히 확인하므로 다른 화면의 미세 변경은 덮어쓰지 않는다.

`manual-weight-explicit-edit-review.md`에 독립 검토와 RED/GREEN을 보존했다.
서버 전체 수동 채점13PASS는16.6667 저장/GET, 동시0.00001 변경의 plan/apply
거부·부분 저장 없음·복구를 포함한다. 화면5PASS는1366/1100/390의 원배점
저장·오류 복구·가독성과1366/390의 직접 배점 수정·저장·재진입을 검증한다.
인접 O/X 자동저장·명시 배점 회귀1PASS, 소유 Vite 종료/5198 listener0 확인.

기존 공식21개 실사용 안에 실제 조교6문항 입력·PC 원배점100점 확정,
390px 배점16.6667 수정/첫 문항0점→83.33 저장·재조회·학생/선택 학부모 결과,
온라인 제출 전후 홈·reload를 추가했다. collection만 완료했고 실제 실행과
300초 내 완료는 아직 미검증이다. 직원 엔트리404와 별개로 외부 QA의 tenant/user
cleanup0가 필수다. 다음은 통합 게이트→새 head CI→배포 소유권 인계→BE 선행
배포→같은 FE 산출물의21개 실사용/cleanup0→운영 승격·영향 화면 확인 순서다.
...
### 현재 실행 상태 — 2026-09-21 KST

- **Backend 운영 배포 완료:** 공식 실행35540757110, 소스 `057403c653`.
- **Frontend 운영 배포 완료:** `8f1d580dc4901631bed5a50eccc60628968c7667`의
  [공식 실행35555250585](https://github.com/guswls3028-art/academy-frontend/actions/runs/35555250585)는
  격리 실사용 **21 PASS/0 SKIP/0 flaky**, 배포와 운영 로그인4개·tenant availability·평가 읽기 전용1개까지 성공했다. 양쪽 QA tenant/user와
  R2 객체·프로세스·포트 잔여0을 확인했다. 실제 배포 파일 지문은
  `38d28718267718d248688cac245695f1de2d381f72ff47da243212f1d1ecf73c`다.
- PR565의 최종 `db56b34e34efc0583c23896f2b33b0f97f52bf4d`는 필수 CI 및
  E2E35555019666 전체 검사를 통과했다: 브라우저740개, iPhone36개
  (기존 플랫폼 전용1개 skip), 운영 읽기 전용6개, bundle/theme 각1개.
  이전 학생 등록 준비 경계 실패는 새 전체 검사에서 재발하지 않았다.
- **최종 화면 QA 완료:** 동일 파일의 단회 v3 실행은 tenant398을 사용해
  PC1366/1100/390 출결 저장·reload, 선생님 화면 PC/390 검색 초점·입력 보존과
  점수31/32 저장·reload를 확인했다. 실제 actor는 admin이며 조교 권한 검증은
  공식21개가 별도로 소유한다. tenant/user/R2/process/listener0, 소유5개 세션
  종료, 파일 불변을 확인했다. 실행 pins는 소비 즉시 false로 회수했다.
- 원본 OMR·재채점과 조교 PC/390의 입력·저장·본인 빈 점유 이전·점수 복원·reload,
  최종 학생/부모 합계까지 통과했다. PC·모바일 영상은 각각690초 재생과 갱신·진도
  저장을 확인했고, 종료 후 활성 재생0·플레이어 오류0이다. 특정 검증 시나리오의
  성공이며 모든 실제 사용자 환경에서 끊김이 없다는 보장은 아니다.
- **공개 운영 확인 완료:** godmin.kr·hakwonplus.com의 기본 URL과 cache-buster URL을
  3회 확인해 버전8f1d와23개 진입 파일·43개 필수 자산의 bytes/hash가 배포 파일과
  일치했다. 전체611개 파일의 네트워크 검증을 뜻하지 않는다. 별도 인증 읽기 검증은
  hakwonplus만 PC1366/390에서 클리닉 진입·달력·대상·23:00→다음 날01:00 입력,
  동일 시각 거부·수정·취소·reload와 overflow0을 확인했다. 조회 API4종200·오류0,
  업무 쓰기 시도/전달0, 로그인 시도/성공1, 관측 쓰기0이다. godmin 인증은 별도
  자격증명이 없어 수행하지 않았다. 운영 생성·예약·되돌리기 쓰기 검증과 혼동하지
  않으며 그 정상 여정은 격리 공식21개가 소유한다.
- **사용 안내:** 숙제는 과제별 **제출관리** 또는 선생님·조교 과제 상세에서 확인한다.
  활성 화면은5초마다 새 제출을 조회하지만 개인 조교에게 자동 알림을 보내지는 않는다.
  파일 열람은 검토 완료가 아니며 명시적인 확인 완료/통과 처리가 필요하다.
  상세 흐름은 [과제 화면](https://github.com/guswls3028-art/academy-frontend/blob/main/docs/TEACHER-SESSION-ASSESSMENTS.md#대기실패빈-상태),
  데이터 계약은 [숙제 채점](../domain/homework-grading.md)을 따른다. 자정 예약·교직원
  직접 등록은 [클리닉](../domain/clinic-booking.md), 본인 점유·수동 점수 보존·재채점은
  [평가 화면](https://github.com/guswls3028-art/academy-frontend/blob/main/docs/ASSESSMENT-OPERATIONS-WORKSPACE.md)과
  [OMR](../domain/omr.md)이 소유한다.
- **인계 절차:** 최종 문서 CI·병합 → completion 소유 작업공간 정리 → 릴리스 소유권 인계.
  Messaging·DS 공용 컨트롤 작업의 별도 병합/runtime HOLD는 인계 전까지 유지한다.
  각 단계의 최종 실행 영수증은 `verification.md`에 추가하며, 게시된 이 절차를 미완료 상태로 해석하지 않는다.
  final QA 뒤 port-lifetime 소유 worktree는 clean·병합 확인 후 공식 Close와 경로 부재를
  확인했다. completion backend/frontend는 문서 인계를 마친 뒤 공식 Close로 정리한다. §4·아래 역사 기록의
  정책 차단 캐시는 삭제0·보존 상태이며 재삭제를 시도하지 않는다.
- 운영 알림 작업35544865053에는 수신 주소 미설정 실패가 있다. 기존 채널을
  확인하기 위한 질문은 아직 응답이 없으며 외부 메시지 전송0·임의 수신처 설정0이다.

최종 영수증은 위 artifact 디렉터리의 `production-8f1d-bundle-readback.json`,
`clinic-production-readonly-1789961555741.json`과 공식 run·최종 QA 증거가 소유한다.
아래의 pending·실패·미실행 표시는 당시 기록이며 이 현재 상태를 대체하지 않는다.

### 실행 경과 — 아래 후보·상태는 당시 기록
...
- 실제49512 모달의 기존 답안 지연 응답으로 저장 key `1,2`를 재현했다.
  ...
  Frontend PR563 최종 후보는 `f79aef5039e043965556895fa10ba3af66b70c4d`다.
  기존 채점 mock은30개 성공했고, 원본49512에서도 실패한 미리보기1개는
