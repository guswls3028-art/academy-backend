...
### AI and Tools worker admission in the stacked candidate

The isolated QA path uses the common `apps.infrastructure.qa_lease.AdmissionGate` only when the exact development QA mode and lease identity are configured. `CANDIDATE_LEASE_REQUIRED=true` without an active verified gate stops enqueue and receive; an explicit false flag leaves ordinary development and production on their existing queue messages and worker flow.

At enqueue, the API's authoritative `AIJobModel.tenant_id` and job ID are used to replace any caller-supplied `_qa_lease` with a signed envelope for the destination AI or Tools queue. The signature also binds the exact outer job type, tier, source fields, creation time and attempt. The same stamped payload is persisted on the job before SQS publication. QA receipt parsing rejects duplicate JSON keys, non-finite numbers, missing or extra envelope fields, type changes and an AI queue tier mismatch. The worker checks the committed lease before and after long polling, validates the raw receipt and claims its signed nonce before any job ORM lookup, then compares the claimed identity, payload, type, tier and source with the persisted job before `prepare_ai_job` or a domain callback. The envelope is removed from the inference contract payload.

For an accepted receipt, the gate records a hash of `job_id:receipt_handle`, tenant, signed message ID and lease revision as in-flight evidence. PREPARED, DRAINING and expired windows keep the worker and its trusted heartbeat alive while pausing receive and product DB/provider work; PREPARED can resume on ACTIVE without a process restart. A lease that closes during an already-started job does not interrupt that job. Callback failure leaves the exact signed message retryable; successful terminal handling durably completes its nonce before SQS acknowledgement. A valid completed redelivery is acknowledged without rerunning inference. An in-flight duplicate or nonterminal job that could not be prepared is deferred without acknowledgement. Expired, changed, untrusted or mismatched receipts are never executed or deleted: an unstarted receipt is released, the process stops receiving, and the trusted activity snapshot records the HOLD. Missing helper, signing key, lease readback or nonce journal fails closed. Queue depth alone is not proof that all workers are idle.

Focused offline verification uses `tests/test_candidate_worker_admission.py` together with `tests/test_candidate_qa_admission.py` for signed enqueue→receive→completion, invalid/foreign receipts, long-poll expiry, duplicate and callback-retry behavior. This candidate-stack code is not live QA evidence; exact-image isolated development still requires the runtime adapter, lease/key grants, tenant/user cleanup and action→save→reload→downstream readback before opening or finishing a bounded QA window.
...
| --- | --- | --- |
| ACTIVE, before `expires_at - 30s` | Fresh gate + tenant/token checks | Allowed within token expiry and tenant authorization |
