...
At enqueue, the API's authoritative `AIJobModel.tenant_id` and job ID are used to replace any caller-supplied `_qa_lease` with a signed envelope for the destination AI or Tools queue. The same stamped payload is persisted on the job before SQS publication. The worker checks the committed lease before and after long polling, then compares the signed receipt with the persisted job's tenant, payload, type, tier and source before `prepare_ai_job` or a domain callback. The envelope is removed from the inference contract payload.

For an accepted receipt, the gate records a hash of `job_id:receipt_handle`, tenant, signed message ID and lease revision as in-flight evidence. A lease that closes during an already-started job does not interrupt that job. Callback failure leaves the exact signed message retryable; successful terminal handling durably completes its nonce before SQS acknowledgement. A valid completed redelivery is acknowledged without rerunning inference. An in-flight duplicate or nonterminal job that could not be prepared is deferred without acknowledgement. Expired, changed, untrusted or mismatched receipts are never executed or deleted: an unstarted receipt is released, the process stops receiving, and the trusted activity snapshot records the HOLD. Missing helper, signing key, lease readback or nonce journal fails closed. Queue depth alone is not proof that all workers are idle.
