...
2. lint, expand/contract migration guard와 smoke가 통과한 뒤 변경 감지 결과에 따라 필요한 이미지(base, api, video-worker, messaging-worker, ai-worker-cpu, tools-worker)만 linux/arm64 빌드해 ECR run-unique `:sha-*` 후보로 푸시한다. 각 worker Dockerfile은 실제 entrypoint import를 build-time에 검증한다. 모든 ECR repo는 scan-on-push이며 새로 빌드한 digest의 scan 완료와 critical finding 0을 요구한다(high은 경고와 remediation 대상으로 남긴다). `:latest`는 이 시점에 움직이지 않는다.
