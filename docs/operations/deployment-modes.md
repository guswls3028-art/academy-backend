...
3. `verify-api-development` job → API/Tools 변경 여부와 무관하게 모든 release candidate에서 같은 manifest의 API/Tools digest를 상시 격리 development에 blue/green 방식으로 배포한다. 전용 DB migration, 운영 DB·R2 접근 거부, 개발 큐/R2/Redis, `/healthz`, `/health`, 이미지 identity와 합성 XLSX/PPT/R2 실사용 smoke가 모두 통과해야 candidate를 active로 승격한다.
   모든 full release 후보는 같은 manifest의 API/Tools digest로 실제 HTTP 생성 요청, development Tools SQS worker, R2 PDF 다운로드·reload, stale retry/terminal callback/중복 재배달/tenant denial과 exact cleanup을 검증한다. 이 canary가 실패하면 preprod에 진입할 수 없다. `workflow_dispatch`의 `deployment_scope=development_only`는 current `origin/main`을 선조로 둔 후보만 허용하고 API/Tools만 선택 빌드하며, `run_wrong_note_canary=true`일 때 같은 canary를 실행한다. 이 scope에서는 `ci-build.latest.md`를 main에 push하지 않으며 `verify-api-preprod`가 구조적으로 skip되어 migration과 모든 production deploy job에 도달할 수 없다.
