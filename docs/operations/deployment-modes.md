...
| 학생 영상 재생 경로 좁은 회귀 | `python scripts/post_deploy_smoke/video_playback_chain.py`. 명시적 `E2E_VIDEO_ID`가 없으면 등록 강의·회차를 순회해 영상이 실제로 있는 첫 회차를 사용하고, 모두 비어 있으면 테넌트 공용 영상 세션을 검증한다. |
