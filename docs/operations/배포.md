...
**원칙**: API/AI/Messaging — ASG Target Tracking (평소 0대, 큐 쌓이면 증설). Video — AWS Batch 전용 (daemon ASG 폐기 2026-05-10).  
**Video Worker 사양**: c6g.4xlarge VCPU=8 / 16GB / 200GB EBS. Batch CE 이름 = `academy-v1-video-batch-ce-200gb`. jobdef timeout 6h.

> ⚠️ **2026-05-11 갱신:** 본 문서 아래 §"Video Worker manual docker run" 류의 명령은 daemon mode 시절 잔재. 현재 영상 인코딩은 AWS Batch 가 jobdef SubmitJob 으로만 실행. EC2 ASG에 docker run 안 함. SSOT: `infrastructure/deployment-architecture.md` + `apps/worker/video_worker/README.md` + `infrastructure/video-cron-jobs.md`.
