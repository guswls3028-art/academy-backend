...
**원칙**: API — ASG 상시 1대 이상. AI — AWS/SQS alarm 기반 ASG StepScaling(평소 0대). Messaging — ASG 상시 1대 이상 + SQS alarm 기반 StepScaling. Video — AWS Batch 전용 (daemon ASG 폐기 2026-05-10).
**Video Worker 사양**: c6g.4xlarge VCPU=8 / 16GB / 200GB EBS. Batch CE 이름 = `academy-v1-video-batch-ce-200gb`. jobdef timeout 6h.
...
| 6 | EC2 API t4g.small 30GB → Docker, ECR pull, .env, migrate, `/healthz` liveness + `/health` readiness |
| 7 | EC2 Messaging t4g.small 상시 (ASG/스크립트는 t4g.small) |
| 8 | Video Batch CE/queue/jobdef 확인 (`academy-v1-video-batch-ce-200gb`, `academy-v1-video-batch-queue`, `academy-v1-video-batch-jobdef`) |
| 9 | AI 워커 ASG(Min=0) + Messaging 워커 ASG(Min=1) |
...
| 방식 | 설명 | 권장 |
|------|------|------|
| **CI 자동 배포** | main push → GitHub Actions가 변경 이미지 빌드·ECR 푸시 → `run-migrations`(필요 시) → `deploy-api`/`deploy-messaging`/`deploy-ai`/`deploy-video` → `verify-deployment`. **push=서버 반영** 자동화. | 일상적인 코드 반영. |
...
| Video | AWS Batch CE (`academy-v1-video-batch-ce-200gb`) |
| AI | ASG Min=0 Max=5 |
