...
**상태:** Cloudflare zone·가비아 위임·정식 배포·운영 DB·무기한 이용·Pages HTTPS 완료 · 대표 표시명 보정 완료 · 대표 계정 생성 대기
...
- [ ] **G7 대표 계정** — 대표 정보는 전달받았고 운영 소유자 수는 여전히
  0명이다. 2026-08-19 KST에 강의 담당자 목록이 비어 있던 현상을 확인해
  `godmin`의 빈 `tenant.owner_name`만 `신민`으로 보정했다. 운영 이미지의
  `GET /api/v1/lectures/lectures/instructor-options/` 뷰를 관리자 요청으로
  재실행해 `200`과 대표 선택지 `신민`을 확인했다. 이 표시명 폴백은 실제
  owner 멤버십이나 로그인 권한을 만들지 않으므로, 인증된 개발자 콘솔에서
  기존 소유자 0명을 다시 확인한 뒤 대표 계정을 1회만 생성한다.
