...
## 3. API 2대 기준선 및 워커 큐 기반 축소 위반 금지

API ASG는 운영 가용성 기준으로 `MinSize=2`, `DesiredCapacity=2`, `MaxSize=3`을 유지한다. Messaging/AI/Tools 워커는 SQS alarm 기반 scale-to-zero가 SSOT이므로 임의로 상시 1대 기준으로 되돌리지 않는다.

```
academy-v1-api-asg              → min=2 desired=2 max=3
...
API min<2는 배포 refresh/인스턴스 교체 중 502 gap을 만들 수 있다. 워커는 SQS가 버퍼 역할을 하므로 scale-to-zero 정책을 깨지 않는 것이 비용·운영 SSOT다.
