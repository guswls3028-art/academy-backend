...
게이트다. 운영과 같은 API 인스턴스 유형·AMI와 동일한 digest-pinned API/Tools 이미지를
...
- Tools worker는 같은 EC2 안의 별도 `academy-tools-development` 컨테이너와 개발 전용
  큐로 상시 실행된다. 운영 Tools ASG의 평시 용량은 변경하지 않는다.
...
- API와 Tools 컨테이너가 동일 release ID의 digest-pinned 이미지로 실행 중
