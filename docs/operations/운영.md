...
## 7. 비디오 인코딩이 진행되지 않을 때 (legacy daemon/SQS 참고)

> 현재 영상 인코딩 운영 절차는 [runbooks/video-batch.md](runbooks/video-batch.md)를 우선한다. 아래 SQS/daemon 점검 절차는 legacy 참고이며, 현재 운영 판단의 SSOT가 아니다.
...
VIDEO_SQS_QUEUE_NAME, INTERNAL_WORKER_TOKEN, SECRET_KEY, LAMBDA_INTERNAL_API_KEY(선택)

**방법 1 – 로컬에 풀 .env 있음**  
PowerShell: `.\scripts\sync_api_env_lambda_internal.ps1` 한 번 실행. (로컬 .env → SSM 올리고, API 서버는 병합 후 컨테이너 재생성.)
