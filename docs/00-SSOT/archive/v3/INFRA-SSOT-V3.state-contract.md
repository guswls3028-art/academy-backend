...
- **단일 진입점:** 모든 배포는 `scripts_v3/deploy.ps1`(Video/Batch 중심) 또는 **전체 스택**은 `scripts_v3/deploy_fullstack.ps1`만 실행한다.
...
## 8. FullStack (deploy_fullstack.ps1)

- **범위:** API(Build)·ASG(Messaging/AI)·Batch(Video/Ops)·RDS·Redis·SSM·ECR·EventBridge·Network를 SSOT v3 기준으로 Ensure/Confirm.
- **진입점:** `scripts_v3/deploy_fullstack.ps1 [-Env prod] [-AllowRebuild] [-SkipNetprobe] [-PruneLegacy]`.
- **순서:** Preflight → Network → IAM → RDS/Redis SG → Batch(CE/Queue/JobDef) → EventBridge → ASG Confirm → SSM/API/Build Confirm → Netprobe → Evidence.
- **ASG:** DesiredCapacity는 **절대 0으로 덮어쓰지 않음**. describe 후 동일 값 유지.
- **API:** EIP(allocationId)로 인스턴스 식별, GET /health 200 확인.
- **Build:** Tag Name=academy-build-arm64 존재/상태 확인. SSM·ssh 검증은 optional.
- **SSOT 외 잔재:** Evidence 출력 시 Batch CE/Queue, EventBridge Rule 중 **SSOT에 없는 이름**을 "SSOT 외 잔재 목록"으로 표시. 삭제는 **-PruneLegacy** 스위치로만 동작(기본 false). Prune 실행 시 확인 프롬프트 권장.
- **현 상태 수집:** `scripts_v3/gather_fullstack_state.ps1` → `docs/00-SSOT/FULLSTACK-CURRENT-STATE-REPORT.md` (변경 없음).
- **Drift 판정:** `scripts_v3/drift_fullstack.ps1` → `docs/00-SSOT/FULLSTACK-DRIFT-TABLE.md` (수정 가능/재생성 필요/수동 확인 3분류).
