...
| 항목 | 값 |
|------|-----|
| 검증 시각 | 2026-03-07T11:13:27.1807533+09:00 |
| 리전 | ap-northeast-2 |
...
| /health 200 | OK 20ms | curl 위 URL 또는 ALB DNS 직접 호출 |
| API 공개 URL(도메인) /health | unreachable | API_PUBLIC_URL 또는 front.domains.api: https://https://api.hakwonplus.com |
...
| Video Batch CE/Queue/JobDef | CE VALID Queue ENABLED JobDef rev 20 | reports/audit.latest.md, Batch Console |
...
| 항목 | 결과 | 근거 |
|------|------|------|
| /health | OK | 응답시간: 20ms (기준 p95 &lt; 2s, 샘플 1회) |
...
- **WARNING** [API] API 공개 URL /health unreachable: https://https://api.hakwonplus.com/healthz — 알려진 호스트가 없습니다. (https:443)
