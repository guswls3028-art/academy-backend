...
## 5b) 실행 정책 (Legacy Kill-Switch)

- **단일 진입점:** 배포는 **scripts_v3/deploy.ps1** 만 실행한다. CI(GitHub Actions) 포함.
- **레거시 차단:** scripts/infra/batch_video_setup.ps1, batch_ops_setup.ps1, eventbridge_deploy_video_scheduler.ps1, cloudwatch_deploy_video_alarms.ps1 를 **직접 실행하면 즉시 실패** (DEPRECATED throw). deploy.ps1에서만 `$env:ALLOW_LEGACY_IMPORT=1` 설정 후 호출 가능.
- **이미지:** Evidence에 ECR imageDigest 기록. `:latest` 당장은 허용하되, Evidence에 digest 필수 출력.
