...
- **도메인:** app/API/CDN 라우팅은 params `front.domains`(추가 예정)로 표준화.
...
- **배포:** `deploy.ps1`에 `-SkipFront` 옵션 및 프론트 단계(빌드 → R2 업로드 → purge → 검증) 추가 또는 별도 `deploy-front.ps1` 호출.
