...
|------|------|-------------------------------------|
| API ASG min/desired/max | not found// | reports/audit.latest.md (apiAsg*) |
| ALB target health | 0 / 0 healthy | AWS Console EC2 > Target Groups > academy-v1-api-tg |
...
| AI/Messaging ASG | / | reports/audit.latest.md (asgAi*, asgMessaging*) |
| SQS queue 연결·DLQ | Messaging depth error DLQ  / AI depth error DLQ  | SQS Console 또는 get-queue-attributes |
| Video Batch CE/Queue/JobDef | CE  Queue  JobDef rev  | reports/audit.latest.md, Batch Console |
| Video Ops CE/Queue, EventBridge | Ops CE  Ops Queue  Reconcile skipped ScanStuck skipped | reports/audit.latest.md, rca.video.latest.md |
| RDS 연결 가능 | not found | RDS describe-db-instances (연결 테스트는 앱/psql 수동) |
| Redis 연결 가능 | not found | ElastiCache describe-replication-groups |
| **섹션 1 종합** | **FAIL** | |
...
| API root | not checked | 동일 ALB DNS |
...
| 항목 | 결과 | 근거 |
|------|------|------|
| 프론트 URL 접속 | not checked | FRONT_APP_URL 또는 SSOT front.domains.app 설정 시 검사 |
| 정적 자산(JS/CSS) 로딩 | 수동 검증 권장 | 브라우저 개발자도구 Network 탭 |
| CDN 캐시 정책 | 수동 검증 권장 | Cache-Control 헤더, 배포 시 purge: SSOT front.purgeOnDeploy |
...
| DLQ 적재 없음 | Messaging DLQ= AI DLQ= | get-queue-attributes ApproximateNumberOfMessages (DLQ) |
| **섹션 4 종합** | **PASS** | |
...
| 항목 | 결과 | 근거 |
|------|------|------|
| 최소 알람 세트(API 5XX, SQS depth/DLQ, Batch failed/stuck/backlog, RDS, Redis) | not listed | CloudWatch > Alarms (academy/v1 필터) |
