...
## 13) OIDC 전용 ECR 빌드·Access Key 워크플로우 폐기 (2026-03-06)

- **워크플로:** `.github/workflows/v1-build-and-push-latest.yml` 신설. OIDC만 사용 (`secrets.AWS_ROLE_ARN_FOR_ECR_BUILD`). setup-qemu + setup-buildx, 5개 이미지(academy-base, academy-api, academy-video-worker, academy-messaging-worker, academy-ai-worker-cpu) linux/arm64 latest 빌드·푸시. push 후 각 repo imageDigest를 ci-build.latest.md에 기록·커밋·artifact 업로드.
- **폐기:** build-and-push-ecr.yml, build-and-push-ecr-nocache.yml, build-and-push-latest.yml 삭제. video_batch_deploy.yml에서 Access Key fallback 제거(OIDC 전용).
- **배포:** API UserData에 배포 nonce·docker pull·stop/rm·run --name academy-api 유지. runtime-images 수집 시 ci-build.latest.md의 academy-api digest와 비교, 불일치 시 "CI vs Runtime: MISMATCH" 명시.
