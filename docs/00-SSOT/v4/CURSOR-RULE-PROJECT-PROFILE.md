...
| 채움 | 선택 | 근거 |
|------|--------|------|
| ⚠️ | **[아래 설명 참고 후 선택]** | 현재 설계: ASG min=1, RDS 단일 인스턴스, Redis replication group. SLA 목표에 따라 아래 중 선택. |

**장애 허용 수준 설명 (선택 시 참고):**

| 수준 | 의미 | 적합한 경우 |
|------|------|-------------|
| **low tolerance** | 장애를 거의 허용하지 않음. 단일 장애로도 서비스 중단·품질 저하를 최소화해야 함. | 금융·결제, 실시간 필수 서비스. Multi-AZ RDS, 다중 인스턴스·헬스체크·자동 복구 강화. |
| **medium** | 일시적 장애·지연은 어느 정도 허용. 단일 AZ/인스턴스 장애 시 복구 시간(수 분~수십 분) 내 수용 가능. | 일반 B2B·학원 관리처럼 단기 중단이 치명적이지 않은 서비스. 현재 Academy와 유사(ASG min=1, 자동 복구). |
| **high** | 장애·지연을 상대적으로 많이 허용. 배치·큐 지연, 일시적 API 오류를 일정 부분 허용. | 내부 도구, 비실시간 배치 위주, 비용 최우선 환경. |

**현재 인프라와의 대응:**  
- API/워커 ASG min=1 → 인스턴스 1대 죽으면 새 인스턴스 기동까지 수 분.  
- RDS 단일 인스턴스 → DB 장애 시 복구는 백업/스냅샷 기준.  
- Redis replication group → 장애 시 복제본 전환 가능.

**질문:** 위 설명을 보고 **low / medium / high** 중 하나를 골라 주세요. (선택하시면 이 문서에 반영합니다.)
