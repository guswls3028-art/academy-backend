...
class SubscriptionViewAuthorizationTests(APITestCase):
    ...
    def test_single_plan_exposes_exact_supply_tax_and_total(self):
        ...
        self.assertEqual(response.data["billing_price_policy"], "legacy_contract")
    ...
    def test_standard_price_exposes_10_percent_vat_and_198000_total(self):
        Program.objects.filter(pk=self.program.pk).update(
            created_at=datetime(2026, 9, 1, tzinfo=dt_timezone.utc),
            monthly_price=180_000,
        )
        self.program.refresh_from_db()
        self._authenticate_owner("standard-price-owner")

        response = self.client.get("/api/v1/core/subscription/", **self.headers)

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["monthly_supply_amount"], 180_000)
        self.assertEqual(response.data["monthly_tax_amount"], 18_000)
        self.assertEqual(response.data["monthly_total_amount"], 198_000)
        self.assertEqual(response.data["vat_rate_percent"], 10)
        self.assertEqual(response.data["list_monthly_total_amount"], 198_000)
    ...
    def test_august_signup_exposes_lifetime_price_guarantee(self):
        ...
        self.assertEqual(response.data["monthly_total_amount"], 159_000)
        self.assertEqual(response.data["list_monthly_total_amount"], 198_000)
        self.assertEqual(response.data["discount_rate"], 19)
