...
class ExamQuestionInitSerializer(serializers.Serializer):
    ...
    question_types = serializers.ListField(
        child=serializers.ChoiceField(choices=["choice", "essay"]),
        required=False,
        allow_empty=False,
        max_length=MAX_EXAM_QUESTIONS,
    )
    ...
    def validate(self, attrs):
        ...
        if question_types is not None:
            if has_total and attrs["total_questions"] != len(question_types):
                raise serializers.ValidationError(
                    "total_questions 는 question_types 길이와 같아야 합니다."
                )
            if has_choice_essay_counts:
                expected_choice = question_types.count("choice")
                expected_essay = question_types.count("essay")
                if choice_count != expected_choice or essay_count != expected_essay:
                    raise serializers.ValidationError(
                        "choice_count/essay_count 는 question_types 구성과 같아야 합니다."
                    )
        ...
        if has_total:
            return attrs
        raise serializers.ValidationError(
            "question_types, total_questions 또는 choice_count/essay_count 를 보내세요."
        )
