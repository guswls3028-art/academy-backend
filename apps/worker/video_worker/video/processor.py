...
def _hls_r2_prefix(cfg: Config, tenant_code: str, video_id: int) -> str:
    base = (cfg.R2_PREFIX or "media/hls").strip("/")
    return f"{base}/{tenant_code}/videos/{int(video_id)}"


def _hls_master_relpath(cfg: Config, tenant_code: str, video_id: int) -> str:
    return f"{_hls_r2_prefix(cfg, tenant_code, video_id)}/master.m3u8"
...
def process_video_job(*, job: Dict[str, Any], cfg: Config, client: VideoAPIClient) -> None:
    ...
    - job은 dict이며, 최소 {"video_id": int, "file_key": str, "tenant_code": str} 기대
    """
    video_id = _safe_int(job.get("video_id"))
    file_key = (job.get("file_key") or "").strip()
    tenant_code = (job.get("tenant_code") or "").strip()

    if not video_id:
        raise KeyError("video_id")
    if not tenant_code:
        raise KeyError("tenant_code")
    ...
    try:
        ...
        with temp_workdir(cfg.TEMP_DIR, prefix=f"video-{video_id}-") as wd:
            ...
            upload_directory(
                ...
                prefix=_hls_r2_prefix(cfg, tenant_code, int(video_id)),
        ...
        client.notify_complete(
            int(video_id),
            {
                "hls_path": _hls_master_relpath(cfg, tenant_code, int(video_id)),
