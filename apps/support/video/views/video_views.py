...
class VideoViewSet(VideoPlaybackMixin, ModelViewSet):
    ...
    def _upload_complete_impl(self, video):
        ...
        # ffprobe 실패(ffmpeg_module_missing, ffprobe_exception_fallback 등) 시 반드시 enqueue
        ...
        if not ok:
            ...
            if not VideoSQSQueue().create_job_and_enqueue(video):
                logger.error("VIDEO_UPLOAD_ENQUEUE_FAILED | video_id=%s | reason=%s", video_id, reason)
                return Response(
