...
MAX_ARCHIVED_IMAGES = 500
MAX_ARCHIVED_IMAGE_TOTAL_BYTES = 1024 * 1024 * 1024
...
def handle_ppt_generation_job(job: AIJob) -> AIResult:
    ...
    try:
        ...
        if mode == "pdf":
            ...
            def _generate_from_images(image_bytes_iter, total_count: int):
                _record_progress(
                ...
                return GeneratePptUseCase().execute(
                    image_bytes_iter,
                    config=config,
                    on_progress=_on_img_progress,
                    total_count=total_count,
                )

            archive_key = payload.get("r2_archive_key")
            if archive_key:
                archive_path = download_r2_key_to_tmp(r2_key=archive_key, job_id=f"{job.id}-archive")
                tmp_dirs.append(os.path.dirname(archive_path))
                with zipfile.ZipFile(archive_path) as zf:
                    ...
                    if not infos:
                        return AIResult.failed(job.id, "image archive is empty")
                    if len(infos) > MAX_ARCHIVED_IMAGES:
                        return AIResult.failed(job.id, f"too many archived images: {len(infos)}")
                    total_uncompressed = sum(info.file_size for info in infos)
                    if total_uncompressed > MAX_ARCHIVED_IMAGE_TOTAL_BYTES:
                        return AIResult.failed(job.id, "image archive exceeds total size limit")
                    ...
                    def _iter_archive_images():
                        for i, info in enumerate(infos):
                            raw_bytes = zf.read(info)
                            yield _apply_image_settings(raw_bytes, settings, i)

                    result = _generate_from_images(_iter_archive_images(), total_imgs)
            else:
                r2_keys = payload.get("r2_keys") or []
                if not r2_keys:
                    return AIResult.failed(job.id, "r2_archive_key or r2_keys required for images mode")
                ...
                result = _generate_from_images(image_bytes_list, len(image_bytes_list))
            pptx_bytes = result.pptx_bytes
            slide_count = result.slide_count
