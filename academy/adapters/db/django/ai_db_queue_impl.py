"""
DB-backed AI Job Queue 구현. .objects. 접근은 이 adapter 내부로 한정 (Gate 7).
"""
...
class DjangoDBJobQueue(JobQueue):
    """SQS-style lease/visibility timeout on DB. ORM 접근은 이 클래스 내부만."""
    ...
    def claim(self, *, worker_id: str) -> Optional[ClaimedJob]:
        ...
        job.save(
            update_fields=[
                ...
                "locked_by", "locked_at", "lease_expires_at", "last_heartbeat_at", "updated_at",
            ]
        )
        return ClaimedJob(
            job_id=job.job_id,
