...
7) 문체 예시는 선생님이 직접 작성하거나 승인한 해설만 들어 있습니다. 어휘 선택, 문장 길이,
   설명 순서와 종결 방식만 참고하고 예시의 고유 사실이나 문장을 복사하지 마세요.
8) 내용 참고 자료는 개념·사실 확인에만 사용합니다. 출판 자료의 문체를 모사하거나 문장을 복사하지 마세요.
9) source_evidence에는 소스 자료에서 근거로 삼은 문항 번호를 최대 3개 넣습니다.
10) answer_check에는 정답을 다시 확인한 짧은 근거를 씁니다.
11) 출력은 JSON 객체만 반환합니다. 코드블록, 설명문, 마크다운을 붙이지 마세요.
