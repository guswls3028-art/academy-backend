...
# 2) Security group for ElastiCache (inbound 6379 from API/Worker SG)
$RedisSgId = aws ec2 describe-security-groups --region $Region --filters "Name=group-name,Values=$RedisSgName" "Name=vpc-id,Values=$VpcId" --query "SecurityGroups[0].GroupId" --output text 2>&1
if (-not $RedisSgId -or $RedisSgId -eq "None") {
    Write-Host "[2/5] Creating security group $RedisSgName ..." -ForegroundColor Cyan
    $RedisSgId = aws ec2 create-security-group --group-name $RedisSgName --description "ElastiCache Redis - allow 6379 from API/Worker" --vpc-id $VpcId --region $Region --output text 2>&1
    aws ec2 authorize-security-group-ingress --group-id $RedisSgId --protocol tcp --port 6379 --source-group $ClientSecurityGroupId --region $Region 2>&1
