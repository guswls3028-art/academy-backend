# ==============================================================================
# One-take: fix Batch worker (IAM + JobDef + post-verify).
...
# 2) IAM – API role (TerminateJob + DescribeJobs)
...
# 3) IAM – Batch job role (reconcile: TerminateJob, DescribeJobs, SubmitJob)
Step "3) IAM – Batch job role (reconcile)"
...
# 4) Job definition – register new revision with current ECR URI
Step "4) Job definition – register revision"
...
# 5) Post-verify – optional TerminateJob probe (production principal)
Step "5) Post-verify"
