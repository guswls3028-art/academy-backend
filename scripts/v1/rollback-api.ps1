...
.DESCRIPTION
    ...
    API 서비스를 이전 버전으로 롤백합니다.
.PARAMETER Sha
    롤백할 SHA 태그 (예: sha-abcd1234). 생략 시 두 번째 최신 SHA 태그를 자동 선택합니다.
.PARAMETER WhatIf
    실제 실행 없이 어떤 작업이 수행될지만 보여줍니다.
.EXAMPLE
    run-with-env.ps1 -- pwsh -File scripts/v1/rollback-api.ps1
    run-with-env.ps1 -- pwsh -File scripts/v1/rollback-api.ps1 -Sha sha-abcd1234
    run-with-env.ps1 -- pwsh -File scripts/v1/rollback-api.ps1 -WhatIf
#>
param(
    [string]$Sha,
    [switch]$WhatIf
...
if ($Sha) {
    ...
    # 자동 선택: 두 번째 최신 sha- 태그 (현재 latest가 아닌 것 중 가장 최신)
...
# manifest를 임시 파일에 저장 (큰 manifest를 CLI 인자로 전달 시 문제 방지)
...
try {
    ...
    # ImageAlreadyExistsException은 이미 같은 이미지를 가리키는 경우 — 정상
...
# Health check (API 전용)
Write-Info "/healthz 확인 중..."
try {
    $healthzResp = Invoke-WebRequest -Uri $HealthzUrl -UseBasicParsing -TimeoutSec 10
    if ($healthzResp.StatusCode -eq 200) {
        Write-OK "/healthz → 200 OK"
    } else {
        Write-Warn "/healthz → $($healthzResp.StatusCode)"
    }
} catch {
    Write-Fail "/healthz 접근 실패: $($_.Exception.Message)"
}

Write-Info "/health 확인 중..."
try {
    $healthResp = Invoke-WebRequest -Uri $HealthUrl -UseBasicParsing -TimeoutSec 10
    if ($healthResp.StatusCode -eq 200) {
        Write-OK "/health → 200 OK (DB 연결 정상)"
    } else {
        Write-Warn "/health → $($healthResp.StatusCode)"
    }
} catch {
    Write-Fail "/health 접근 실패: $($_.Exception.Message)"
}

# ASG 인스턴스 상태
Write-Info "ASG 인스턴스 상태 확인 중..."
