<#
.SYNOPSIS
    AI Worker 서비스 롤백 스크립트
.DESCRIPTION
    ECR 이미지의 SHA 태그를 :latest로 재태깅하고 ASG Instance Refresh를 실행하여
    AI Worker 서비스를 이전 버전으로 롤백합니다.
.PARAMETER Sha
    롤백할 SHA 태그 (예: sha-abcd1234). 생략 시 두 번째 최신 SHA 태그를 자동 선택합니다.
.PARAMETER WhatIf
    실제 실행 없이 어떤 작업이 수행될지만 보여줍니다.
.EXAMPLE
    run-with-env.ps1 -- pwsh -File scripts/v1/rollback-ai.ps1
    run-with-env.ps1 -- pwsh -File scripts/v1/rollback-ai.ps1 -Sha sha-abcd1234
    run-with-env.ps1 -- pwsh -File scripts/v1/rollback-ai.ps1 -WhatIf
#>
param(
    [string]$Sha,
    [switch]$WhatIf
...
Write-Info "AI Worker는 HTTP 엔드포인트가 없습니다. ASG 인스턴스 상태를 확인합니다."
...
Write-Info ""
Write-Info "추가 확인 권장:"
Write-Info "  - SQS 큐 깊이 확인 (AI 작업이 정상 처리되는지)"
