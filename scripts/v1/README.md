...
- **ECR cleanup fail-closed**: cleanup은 ASG/LT와 실제 desired InService 컨테이너 `RepoDigests`, 정확히 8개인 Video ACTIVE job definition, 마지막 complete/successful 6-image manifest(공통 base 포함)를 먼저 보호한다. autoscaling 직후 `InService` 인스턴스의 Docker/container bootstrap이 아직 끝나지 않았으면 실제 digest를 최대 10분 동안 제한 재시도하며, 그 안에도 inventory가 완결되지 않거나 부분 삭제 실패·verify 경고가 하나라도 있으면 이미지를 삭제하지 않고 nonzero로 종료한다.
