...
if (-not $s3GwExists -and $FixMode) {
    ...
    & aws ec2 create-vpc-endpoint --vpc-id $VpcId --vpc-endpoint-type Gateway --service-name "com.amazonaws.$Region.s3" --route-table-ids $rtIdsForS3 --region $Region 2>&1 | Out-Null
