...
function Test-JobDefDrift {
    ...
    $rCur = if ($Current.retryStrategy) { $Current.retryStrategy.attempts } else { $null }
    $rDes = if ($Desired.retryStrategy) { $Desired.retryStrategy.attempts } else { $null }
    if ($rCur -ne $rDes) { return $true }
    $pCur = if ($Current.platformCapabilities) { ($Current.platformCapabilities | Sort-Object) | ConvertTo-Json -Compress } else { "" }
    $pDes = if ($Desired.platformCapabilities) { ($Desired.platformCapabilities | Sort-Object) | ConvertTo-Json -Compress } else { "" }
    if ($pCur -ne $pDes) { return $true }
    $eCur = if ($c.environment) { ($c.environment | ConvertTo-Json -Compress) } else { "[]" }
    $eDes = if ($d.environment) { ($d.environment | ConvertTo-Json -Compress) } else { "[]" }
    if ($eCur -ne $eDes) { return $true }
    return $false
}
