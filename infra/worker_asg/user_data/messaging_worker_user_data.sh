#!/bin/bash
# ASG Messaging Worker: Docker + ECR pull + run academy-messaging-worker
# EC2_IDLE_STOP_THRESHOLD=0 → self-stop 비활성화 (ASG가 scale-in으로 종료)
# Min=1 → 항시 1대 대기
set -e
yum update -y
yum install -y docker
yum install -y ec2-instance-connect 2>/dev/null || true
systemctl start docker && systemctl enable docker

ENV_FILE="/opt/academy/.env"
mkdir -p /opt/academy
aws ssm get-parameter --name /academy/workers/env --with-decryption --query Parameter.Value --output text --region ap-northeast-2 > "$ENV_FILE" 2>/dev/null || true

ECR="{{ECR_REGISTRY}}"
aws ecr get-login-password --region ap-northeast-2 | docker login --username AWS --password-stdin "$ECR"
docker pull "$ECR/academy-messaging-worker:latest"
docker stop academy-messaging-worker 2>/dev/null || true
docker rm academy-messaging-worker 2>/dev/null || true

# docker run 재시도 (실패 시 10초 후 최대 3회)
for i in 1 2 3; do
  docker rm -f academy-messaging-worker 2>/dev/null || true
  if docker run -d --name academy-messaging-worker --restart unless-stopped \
    --log-driver awslogs --log-opt awslogs-region=ap-northeast-2 --log-opt awslogs-group=/aws/ec2/academy-messaging-worker \
    --env-file "$ENV_FILE" \
    -e DJANGO_SETTINGS_MODULE=apps.api.config.settings.worker \
    -e EC2_IDLE_STOP_THRESHOLD=0 \
    "$ECR/academy-messaging-worker:latest"; then
    break
  fi
  echo "docker run attempt $i failed, retrying in 10s..."
  sleep 10
done

# 결과 확인 (cloud-init-output.log에 남음, 디버깅용)
docker ps -a
echo "Messaging worker user data done"
